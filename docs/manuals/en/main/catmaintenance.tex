...
One way to create the database would be calling the bareos database preparation scripts with the \parameter{--host} parameter,
as well as adding the database hostname/address as \linkResourceDirective{Dir}{Catalog}{DB Address}. 
How ever, it is advised to configure the \bareosDir \nameref{DirectorResourceCatalog} and grant the \user{postgres} user access.

As the Unix user \user{postgres} has, by default, no access to the bareos configuration path you need to add it to the \group{bareos} by:
\begin{commands}{Add the posgres user to bareos user group}
sudo usermod -aG bareos postgres
\end{commands}

Now, you should set the PostgreSQL server IP as \linkResourceDirective{Dir}{Catalog}{DB Address} in \nameref{DirectorResourceCatalog}.
...
Now, run the Bareos database preparation scripts: 
\begin{commands}{Setup Bareos catalog database}
su - postgres
/usr/lib/bareos/scripts/create_bareos_database
/usr/lib/bareos/scripts/make_bareos_tables
